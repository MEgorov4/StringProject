...
TEST(NStringPlayground, assignmentFromNullShouldThrow)
{
	...
	ASSERT_THROW(nstring::String s(nullCStr), std::invalid_argument);
}
...
TEST(NStringPlayground, largeResizeShouldThrow)
{
	...
	ASSERT_THROW(s.resize(nstring::String::MAX_CAPACITY + 1), std::length_error);
}
...
TEST(NStringPlayground, randomAppendChainPreservesSize)
{
	...
	for (size_t i = 0; i < 100; ++i)
	{
		...
		nstring::String frag(part.c_str());
